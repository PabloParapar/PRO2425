package src.ud2.explogicas;

import java.text.DecimalFormat;
import java.util.*;

public class Boletin23 {
    public static void main(String[] args) {

        //MENÚ EJERCICIOS
        menuEjercicios();
        System.out.println("        Introduzca el número del ejercicio que desea ejecutar: ");
        Scanner boletin23 = new Scanner(System.in);
        int numeroEjercicio = boletin23.nextInt();
        while (numeroEjercicio != 0) {
            switch (numeroEjercicio) {

                //ESTRUCTURA REPETITIVA FOR

                //EJERCICIO 1
                case 1:
                    ejercicio1();
                    break;

                //EJERCICIO 2
                case 2:
                    ejercicio2(boletin23);
                    break;

                //EJERCICIO 3
                case 3:
                    ejercicio3(boletin23);
                    break;

                //EJERCICIO 4
                case 4:
                    ejercicio4(boletin23);
                    break;

                //EJERCICIO 5
                case 5:
                    ejercicio5(boletin23);
                    break;

                //EJERCICIO 6
                case 6:
                    ejercicio6(boletin23);
                    break;

                //EJERCICIO 7
                case 7:
                    ejercicio7(boletin23);
                    break;

                //EJERCICIO 8
                case 8:
                    ejercicio8(boletin23);
                    break;

                //EJERCICIO 9
                case 9:
                    ejercicio9(boletin23);
                    break;

                //EJERCICIO 10
                case 10:
                    ejercicio10(boletin23);
                    break;

                //EJERCICIO 11
                case 11:
                    ejercicio11(boletin23);
                    break;

                //EJERCICIO 12
                case 12:
                    ejercicio12(boletin23);
                    break;

                //EJERCICIO 13
                case 13:
                    ejercicio13(boletin23);
                    break;

                //EJERCICIO 14
                case 14:
                    ejercicio14(boletin23);
                    break;

                //EJERCICIO 15
                case 15:
                    ejercicio15();
                    break;

                //EJERCICIO 16
                case 16:
                    ejercicio16(boletin23);
                    break;

                //EJERCICIO 17
                case 17:
                    ejercicio17();
                    break;
                //EJERCICIO 18
                case 18:
                    ejercicio18(boletin23);
                    break;

                //EJERCICIO 19
                case 19:
                    ejercicio19(boletin23);
                    break;

                //EJERCICIO 20
                case 20:
                    ejercicio20(boletin23);
                    break;

                //EJERCICIO 21
                case 21:
                    ejercicio21(boletin23);
                    break;

                //EJERCICIO 22
                case 22:
                    ejercicio22(boletin23);
                    break;

                //EJERCICIO 23
                case 23:
                    ejercicio23(boletin23);
                    break;

                //EJERCICIO 24
                case 24:
                    ejercicio24(boletin23);
                    break;

                //ESTRUCTURA REPETITIVA WHILE

                //EJERCICIO 25
                case 25:
                    ejercicio25();
                    break;

                //EJERCICIO 26
                case 26:
                    ejercicio26(boletin23);
                    break;

                //EJERCICIO 27
                case 27:
                    ejercicio27(boletin23);
                    break;

                //EJERCICIO 28
                case 28:
                    ejercicio28(boletin23);
                    break;

                //EJERCICIO 29
                case 29:
                    ejercicio29(boletin23);
                    break;

                //EJERCICIO 30 | Me está dando error el segundo método para valores de la forma númeroPrimo^2
                case 30:
                    ejercicio30(boletin23);
                    break;

                //EJERCICIO 31
                case 31:
                    ejercicio31(boletin23);
                    break;

                //EJERCICIO 32 | Muestra 51 porque el 1 no se considera primo.
                case 32:
                    ejercicio32();
                    break;

                //EJERCICIO 33
                case 33:
                    ejercicio33(boletin23);
                    break;

                //EJERCICIO 34
                case 34:
                    ejercicio34(boletin23);
                    break;

                //ESTRUCTURA REPETITIVA DO-WHILE

                //EJERCICIO 35
                case 35:
                    ejercicio35();
                    break;

                //EJERCICIO 36
                case 36:
                    ejercicio36(boletin23);
                    break;

                //EJERCICIO 37
                case 37:
                    ejercicio37();
                    break;

                //EJERCICIO 38
                case 38:
                    ejercicio38();
                    break;

                //EJERCICIO 39
                case 39:
                    ejercicio39();
                    break;

                //EJERCICIOS DE ESTRUCTURA REPETITIVA LIBRE

                //EJERCICIO 40
                case 40:
                    ejercicio40(boletin23);
                    break;

                //EJERCICIO 41
                case 41:
                    ejercicio41(boletin23);
                    break;

                //EJERCICIO 42
                case 42:
                    ejercicio42(boletin23);
                    break;

                //EJERCICIO 43
                case 43:
                    ejercicio43(boletin23);
                    break;

                //EJERCICIO 44
                case 44:
                    ejercicio44();
                    break;

                //EJERCICIO 45
                case 45:
                    ejercicio45();
                    break;

                //EJERCICIO 46
                case 46:
                    ejercicio46();
                    break;

                //EJERCICIO 47
                case 47:
                    ejercicio47();
                    break;

                //EJERCICIO 48
                case 48:
                    ejercicio48();
                    break;

                default:
                    menuEjercicios();
                    System.out.println("Ese no sale en el boletin, escoge otro: \n");
                    break;
            }
            System.out.println("\nIngresa el siguiente ejercicio:");
            numeroEjercicio = boletin23.nextInt();
        }
    }

    private static void menuEjercicios() {
        String reset = "\u001b[0m";         //Reset
        String snooze = "\033[30m";         //Black
        String saltado = "\033[31m";        //Red
        String terminado = "\033[32m";      //Green
        String incubadora = "\033[35m";     //Magenta
        String enProceso = "\33[33m";       //Yellow
        String destacado = "\033[36m";      //Cyan

        System.out.println("\n                     Unidad Didáctica 2 | Boletín 2 3\n\n" +
                "        ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓\n" +
                "        ┃               Estructura repetitiva " + destacado + "for" + reset + "               ┃\n" +
                "        ┠──────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┨\n" +
                "        ┃   " + terminado + "1" + reset + "  │   " + terminado + "2" + reset + "  │   " + terminado + "3" + reset + "  │   " + terminado + "4" + reset + "  │   " + terminado + "5" + reset + "  │   " + terminado + "6" + reset + "  │   " + terminado + "7" + reset + "  │   " + terminado + "8" + reset + "  ┃\n" +
                "        ┠──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┨\n" +
                "        ┃   " + terminado + "9" + reset + "  │  " + terminado + "10" + reset + "  │  " + terminado + "11" + reset + "  │  " + terminado + "12" + reset + "  │  " + terminado + "13" + reset + "  │  " + terminado + "14" + reset + "  │  " + terminado + "15" + reset + "  │  " + terminado + "16" + reset + "  ┃\n" +
                "        ┠──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┨\n" +
                "        ┃  " + terminado + "17" + reset + "  │  " + terminado + "18" + reset + "  │  " + terminado + "19" + reset + "  │  " + terminado + "20" + reset + "  │  " + terminado + "21" + reset + "  │  " + terminado + "22" + reset + "  │  " + terminado + "23" + reset + "  │  " + terminado + "24" + reset + "  ┃\n" +
                "        ┣━━━━━━┷━━━━━━┷━━━━━━┷━━━━━━┷━━━━━━┷━━━━━━┷━━━━━━┷━━━━━━┫\n" +
                "        ┃              Estructura repetitiva " + destacado + "while" + reset + "              ┃\n" +
                "        ┠───────────┬──────────┬──────────┬──────────┬──────────┨\n" +
                "        ┃     " + terminado + "25" + reset + "    │    " + terminado + "26" + reset + "    │    " + terminado + "27" + reset + "    │    " + terminado + "28" + reset + "    │    " + terminado + "29" + reset + "    ┃\n" +
                "        ┠───────────┼──────────┼──────────┼──────────┼──────────┨\n" +
                "        ┃     " + terminado + "30" + reset + "    │    " + terminado + "31" + reset + "    │    " + terminado + "32" + reset + "    │    " + terminado + "33" + reset + "    │    " + terminado + "34" + reset + "    ┃\n" +
                "        ┣━━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┫\n" +
                "        ┃             Estructura repetitiva " + destacado + "do-while" + reset + "            ┃\n" +
                "        ┠───────────┬──────────┬──────────┬──────────┬──────────┨\n" +
                "        ┃     " + terminado + "35" + reset + "    │    " + terminado + "36" + reset + "    │    " + terminado + "37" + reset + "    │    " + terminado + "38" + reset + "    │    " + terminado + "39" + reset + "    ┃\n" +
                "        ┣━━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┫\n" +
                "        ┃       Ejercicios de estructura repetitiva libre       ┃\n" +
                "        ┠───────────┬──────────┬──────────┬──────────┬──────────┨\n" +
                "        ┃     " + terminado + "40" + reset + "    │    " + terminado + "41" + reset + "    │    " + terminado + "42" + reset + "    │    " + enProceso + "43" + reset + "    │    " + incubadora + "44" + reset + "    ┃\n" +
                "        ┠───────────┼──────────┼──────────┼──────────┼──────────┨\n" +
                "        ┃     " + incubadora + "45" + reset + "    │    " + incubadora + "46" + reset + "    │    " + incubadora + "47" + reset + "    │    " + incubadora + "48" + reset + "    │     0    ┃\n" +
                "        ┗━━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┷━━━━━━━━━━┛\n\n");
    }

    private static void ejercicio1() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 1
                Calcular el Sumatorio de los primeros 100 números naturales
                """);
        int n = 0;
        for (int i = 1; i <= 100; i++) {
            n += i;
        }
        System.out.println("La suma de los primeros 100 números naturales es " + n);
    }

    private static void ejercicio2(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 2
                Calcular la media de 5 números dados por el usuario
                """);

        int numero = 0;
        System.out.println("Introduzca 5 números para obtener la media:");
        for (int i = 5; i > 0; i--) {
            System.out.println("Quedan " + i + " números por introducir: ");
            numero += escaner.nextInt();
        }
        System.out.println("La media es " + (numero / 5.0));
    }

    private static void ejercicio3(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 3
                Calcular la media de tantos números como especifique el usuario
                """);

        System.out.println("¿A cuántos números le harás la media?");
        int i3 = escaner.nextInt();
        int numero = 0;
        for (int i = i3; i > 0; i--) {
            System.out.println("Introduzca un número, quedan " + i + " números por introducir: ");
            numero += escaner.nextInt();
        }
        System.out.println("La media entre los " + i3 + " números dados es: " + ((double) numero / i3));
    }

    private static void ejercicio4(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 4
                Dados 2 números, sumar los pares dentro del intervalo entre ellos
                """);

        System.out.println("Introduzca 2 valores para sumar los números pares del intervalo que forman: ");
        int i4inicio = escaner.nextInt();
        int i4fin = escaner.nextInt();
        int sumasPares = 0;
        if (i4inicio < i4fin) {
            for (int i = i4inicio; i <= i4fin; i++) {
                if (i % 2 == 0) {
                    sumasPares += i;
                }
            }
        } else {
            for (int i = i4fin; i <= i4inicio; i++) {
                if (i % 2 == 0) {
                    sumasPares += i;
                }
            }
        }
        System.out.println("La suma de los números pares entre " + i4inicio + " y " + i4fin + " es " + sumasPares);
    }

    private static void ejercicio5(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 5
                Sumar los primeros 'n' términos de la serie armónica siendo 'n' especificado por el usuario
                """);
        System.out.println("¿Cuántos términos de la suma armónica quiere sumar?");

        int i5 = escaner.nextInt();
        double serieArmonica = 0;
        for (int i = 1; i <= i5; i++) {
            serieArmonica += (double) 1 / i;
        }
        System.out.println("Resultado de la suma de los primeros " + i5 + " términos de la serie armónica:\n" + serieArmonica);
    }

    private static void ejercicio6(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 6
                Sumar los primeros 'n' términos de la serie dada, siendo 'n' especificado por el usuario
                """);

        System.out.println("¿Cuántos términos de la serie quiere sumar?");
        int i6 = escaner.nextInt();
        double sumaSerie = 0;

        for (int i = 0; i <= i6 - 1; i++) {
            if (i == 0) {
                sumaSerie = 0;
            } else {
                sumaSerie += (double) (5 * i) / (2 * Math.pow(3, i));
                //System.out.println(sumaSerie);
            }
        }
        System.out.println("La suma de los primeros " + i6 + " términos de la serie es igual a " + sumaSerie);
    }

    private static void ejercicio7(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 7
                Calcular la suma de los primeros 'n' términos de la serie dada, siendo 'n' especificado por el usuario""");

        System.out.println("¿Cuántos términos de la serie quiere sumar?");
        int i7 = escaner.nextInt();
        double sumaSerie7 = 0;

        for (int i = 1; i <= i7; i++) {
            sumaSerie7 += i / Math.pow(2, i);
            //System.out.println(sumaSerie7);
        }
        System.out.println("Resultado de la suma de los primeros " + i7 + " términos de la serie:\n" + sumaSerie7);
    }

    private static void ejercicio8(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 8
                Genera una tabla de conversión de temperaturas de Fahrenheit a Celsius, Rankine y Kelvin
                """);

        System.out.println("Introduzca los grados en Fahrenheit (desde / hasta):");
        int desde = escaner.nextInt();
        int hasta = escaner.nextInt();
        float temperaturaC;
        float temperaturaR;
        float temperaturaK;
        DecimalFormat tabla = new DecimalFormat("##0.00");
        if (desde < hasta) {
            System.out.println("Temperaturas desde " + desde + "Fº hasta " + hasta + "Fº:\n┏━━━━━━━━━━━━━━━┯━━━━━━━━━━━┯━━━━━━━━━━━┯━━━━━━━━━━━┓\n┃\tFahrenheit\t│\tCelsius\t│\tRankine\t│\tKelvin\t┃\n┠───────────────┼───────────┼───────────┼───────────┨");
            for (int i = desde; i <= hasta; i++) {
                temperaturaC = (float) (5 * (i - 32)) / 9;
                temperaturaR = (float) (i + 459.67);
                temperaturaK = (float) (temperaturaC + 273.15);
                System.out.println("┃\t" + tabla.format(i) + "\t\t│\t" + tabla.format(temperaturaC) + "\t│\t" + tabla.format(temperaturaR) + "\t│\t" + tabla.format(temperaturaK) + "\t┃");
            }
        } else {
            System.out.println("Temperaturas desde " + hasta + "Fº hasta " + desde + "Fº:\n┏━━━━━━━━━━━━━━━┯━━━━━━━━━━━┯━━━━━━━━━━━┯━━━━━━━━━━━┓\n┃\tFahrenheit\t│\tCelsius\t│\tRankine\t│\tKelvin\t┃\n┠───────────────┼───────────┼───────────┼───────────┨");
            for (int i = hasta; i <= desde; i++) {
                temperaturaC = (float) (5 * (i - 32)) / 9;
                temperaturaR = (float) (i + 459.67);
                temperaturaK = (float) (temperaturaC + 273.15);
                System.out.println("┃\t" + tabla.format(i) + "\t\t│\t" + tabla.format(temperaturaC) + "\t│\t" + tabla.format(temperaturaR) + "\t│\t" + tabla.format(temperaturaK) + "\t┃");
            }
        }
        System.out.println("┗━━━━━━━━━━━━━━━┷━━━━━━━━━━━┷━━━━━━━━━━━┷━━━━━━━━━━━┛");
    }

    private static void ejercicio9(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 9 a)
                Calcula el factorial de un número dado por el usuario
                """);
        System.out.println("¿De qué número quiere calcular su factorial?");
        int factorial = escaner.nextInt();
        long resultadoFactorial = 1;
        switch (factorial) {
            case 0, 1:
                System.out.println("El Factorial de " + factorial + " es 1");
                break;
            default:
                for (int i = 1; i <= factorial; i++) {
                    resultadoFactorial *= i;
                }
                System.out.println("El Factorial de " + factorial + "! es " + resultadoFactorial);
                break;
        }
        System.out.println("""
                    Boletin 2 3 | Ejercicio 9 b)
                Calcula el factorial de los 20 primeros números naturales
                """);
        long factorial20 = 1;
        for (int i = 1; i <= 20; i++) {
            factorial20 *= i;
            System.out.println(i + "!\t=\t" + factorial20);
        }
    }

    private static void ejercicio10(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 10
                Lista los 'n' primeros números de la serie 1, -2, 4, -8... dado 'n' por el usuario
                """);
        System.out.println("¿Cuándos números de la serie quiere listar?");
        int numeroSerie = escaner.nextInt();
        for (int i = 0; i <= numeroSerie; i++) {
            System.out.println((long) Math.pow(-2, i));
        }
    }

    private static void ejercicio11(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 11
                Dados 'n' números por el usuario, indica el mayor, el menor y la media
                """);

        double mediaNumero = 0;
        System.out.println("¿Cuántos números quiere procesar?\n");
        int cantidad = escaner.nextInt();
        System.out.println("Indica cuáles son: \n");
        int numeroMayor = 0;
        int numeroMenor = 0;
        for (int i = 1; i <= cantidad; i++) {
            int numeroEntrada = escaner.nextInt();
            mediaNumero += numeroEntrada;
            if (i == 1) {
                numeroMayor = numeroEntrada;
                numeroMenor = numeroEntrada;
            } else {
                if (numeroMayor < numeroEntrada) {
                    numeroMayor = numeroEntrada;
                } else if (numeroMenor > numeroEntrada) {
                    numeroMenor = numeroEntrada;
                }
            }
        }
        System.out.println("Número Mayor\t=\t" + numeroMayor + "\nNúmero Menor\t=\t" + numeroMenor + "\nMedia Aritmética\t=\t" + mediaNumero / cantidad);
    }

    private static void ejercicio12(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 12 a)
                Dado un número entre 1 y 9 muestra su tabla de multiplicar
                """);
        System.out.println("¿De qué número desea saber su tabla de multiplicar?");
        int numeroTabla = escaner.nextInt();
        if (numeroTabla >= 1 && numeroTabla <= 9) {
            for (int i = 1; i <= 10; i++) {
                System.out.println(numeroTabla + " * " + i + " = " + (numeroTabla * i));
            }
        } else {
            System.out.println("El número no es correcto");
        }
        System.out.println("""
                    Boletin 2 3 | Ejercicio 12 b)
                Lista las tablas de multiplicar del 1 al 9
                """);

        for (int i = 1; i <= 9; i++) {
            System.out.println("┏━━━━━━━━━━━━━━━━━━━┓\n┃\t  Tabla del " + i + "  \t┃\n┠───────────────────┨");
            for (int j = 1; j <= 10; j++) {
                System.out.println("┃\t" + i + " * " + j + "\t=\t" + (i * j) + "\t┃");
            }
            System.out.println("┗━━━━━━━━━━━━━━━━━━━┛\n\n");
        }
    }

    private static void ejercicio13(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 13
                Calcula el siguiente sumatorio y productorio hasta un numero 'n' dado por el usuario
                """);
        int numeroSumatorio = escaner.nextInt();
        double terminoSumatorio;
        double sumatorio = 0;
        double productorio = 1;
        for (int i = 1; i <= numeroSumatorio; i++) {
            terminoSumatorio = 1.0 / i;
            if (i >= numeroSumatorio) {
                for (int j = numeroSumatorio; j <= i; j++) {
                    productorio *= j;
                    //System.out.println(i + ", " + j + ", productorio: " + productorio);
                }
            }
            sumatorio += terminoSumatorio + productorio;
            //System.out.println(i + ", sumatorio: " + sumatorio);
        }
        System.out.println("Resultado: " + sumatorio);
                /*
                A efectos prácticos, se reduce al sumatorio de ( 1 / i ) + n
                 */
    }

    private static void ejercicio14(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 14
                Muestra en pantalla todos los divisores de un número 'n' dado por el usuario
                """);
        System.out.println("¿De qué número desea saber sus divisores?");
        int numeroDivisores = escaner.nextInt();
        System.out.println("Los divisores de " + numeroDivisores + " son:");
        for (int i = numeroDivisores; i >= 1; i--) {
            if (numeroDivisores % i == 0) {
                System.out.println("\t· " + i);
            }
        }
    }

    private static void ejercicio15() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 15
                Calcula el cuadrado de los 10 primeros números pares
                """);

        System.out.println("El cuadrado de los primeros 10 números pares son:");
        for (int i = 2; i <= 20; i += 2) {
            System.out.println("\t· " + (int) Math.pow(i, 2));
        }
    }

    private static void ejercicio16(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 16
                Dadas por el usuario una base y un exponente máximo calcular las potencias de la base hasta llegar al exponente máximo
                """);
        System.out.println("¿De qué número quiere calcular las potencias?");
        int base = escaner.nextInt();
        System.out.println("Indique la potencia más alta a la que desea elevar " + base);
        int potencia = escaner.nextInt();
        for (int i = 1; i <= potencia; i++) {
            System.out.println(base + " ^ " + i + " = " + (int) Math.pow(base, i));
        }
    }

    private static void ejercicio17() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 17
                Comprobar la velocidad de dos códigos distintos para efectuar la misma tarea y mostrar el más rápido
                """);

        long tiempoInicio1 = System.currentTimeMillis();
        for (int i = 1; i <= 1000000; i++) {
            if (i % 2 == 0) {
                System.out.println("Par");
            } else {
                System.out.println("Impar");
            }
        }
        long tiempoFinal1 = System.currentTimeMillis();
        long tiempoInicio2 = System.currentTimeMillis();
        for (int i = 1; i <= 1000000; i++) {
            if ((i & 1) == 0) {
                System.out.println("Par");
            } else {
                System.out.println("Impar");
            }
        }
        long tiempoFinal2 = System.currentTimeMillis();
        System.out.println("Método 1:\nTiempo inicio = " + tiempoInicio1 + "\tTiempo final = " + tiempoFinal1 + "\t\tTiempo total = " + (tiempoFinal1 - tiempoInicio1) + "\n\nMétodo 2:\nTiempo inicio = " + tiempoInicio2 + "\tTiempo final = " + tiempoFinal2 + "\t\tTiempo total = " + (tiempoFinal2 - tiempoInicio2));
        if (tiempoFinal1 - tiempoInicio1 > tiempoFinal2 - tiempoInicio2) {
            System.out.println("El segundo método es más rápido");
        } else {
            System.out.println("El primer método es más rápido");
        }
    }

    private static void ejercicio18(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 18
                Dado un rango, calcular los cubos de los impares dentro del rango
                """);
        System.out.println("Indica el primer número del intervalo a comprobar");
        int inicioIntervaloCubo = escaner.nextInt();
        System.out.println("Indica el último número del intervalo a comprobar");
        int finalIntervaloCubo = escaner.nextInt();
        for (int i = inicioIntervaloCubo; (i <= finalIntervaloCubo) && (i - inicioIntervaloCubo < 20); i++) {
            if ((i & 1) != 0) {
                System.out.println("\t· " + (long) Math.pow(i, 3));
            }
        }
    }

    private static void ejercicio19(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 19
                Dados dos años, calcular los años bisiestos dentro del rango de los años dados
                """);
        System.out.println("Indica el año del inicio del intervalo a comprobar");
        int anhoInicio = escaner.nextInt();
        System.out.println("Indica el año del final del intervalo a comprobar");
        int anhoFinal = escaner.nextInt();
        System.out.println("Los años comprendidos entre " + anhoInicio + " y " + anhoFinal + " son:");
        for (int i = anhoInicio; i <= anhoFinal; i++) {
            if (i % 4 == 0) {
                if (i % 100 == 0) {
                    if (i % 400 == 0) {
                        System.out.println("\t· " + i);
                    } //else no es bisiesto
                } else { //es bisiesto
                    System.out.println("\t· " + i);
                }
            } //else no es bisiesto
        }
    }

    private static void ejercicio20(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 20
                Mostrar en pantalla el triángulo de Floyd ascendente y descendente dado el número de filas
                """);
        System.out.println("Indica cuántas filas tendrá el triángulo de Floyd");
        int filasFloyd = escaner.nextInt();
        int numeroFloyd = 1;
        System.out.println("\nAscendente\n");
        for (int i = 1; i <= filasFloyd; i++) {
            for (int j = 1; j <= i; j++) {
                System.out.print("\t" + numeroFloyd);
                numeroFloyd++;
            }
            System.out.println();
        }
        System.out.println("\nDescendente\n");
        numeroFloyd = 1;
        for (int i = 1; i <= filasFloyd; i++) {
            for (int j = filasFloyd; j >= i; j--) {
                System.out.print("\t" + numeroFloyd);
                numeroFloyd++;
            }
            System.out.println();
        }
    }

    private static void ejercicio21(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 21
                Muestra en pantalla un menú con opciones indicadas por un número
                """);

        System.out.println("\t1.\tCuadrado.\n\t2.\tRectángulo.\n\t3.\tTriángulo\n\t4.\tCírculo.\n\t0.\tSalir.\n\nSeleccione una opción del menú");
        int menu = escaner.nextInt();
        switch (menu) {
            case 1:
                System.out.println("Cuadrado.");
                break;
            case 2:
                System.out.println("Rectángulo.");
                break;
            case 3:
                System.out.println("Triángulo.");
                break;
            case 4:
                System.out.println("Círculo");
                break;
            case 0:
                System.out.println("Salir");
                break;
            default:
                System.out.println("ERROR | OPCIÓN NO VÁLIDA");
        }
    }

    private static void ejercicio22(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 22
                Calcular el cuadrado de un número 'n' dado por el usuario, usando únicamente sumas
                """);
        int numeroBase = escaner.nextInt();
        int numeroCuadrado = 0;

        for (int i = 0; i < numeroBase; i++) {
            numeroCuadrado += (1 + 2 * i);
        }
        System.out.println("El cuadrado de " + numeroBase + " es " + numeroCuadrado);
    }

    private static void ejercicio23(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 23
                Multiplicar dos números sin usar el operador de multiplicación
                """);
        System.out.println("Ingresa los 2 números a multiplicar:");
        int producto1 = escaner.nextInt();
        int producto2 = escaner.nextInt();
        int productoFinal = 0;
        for (int i = 1; i <= producto2; i++) {
            productoFinal += producto1;
        }
        System.out.println("El producto entre " + producto1 + " y " + producto2 + " es igual a " + productoFinal);
    }

    private static void ejercicio24(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 24
                Calcular la división entera y resto entre dos números dados sin utilizar los operadores de división y módulo
                """);
        System.out.println("Indica un Dividendo: ");
        int entradaDividendo = escaner.nextInt();
        int dividendo = entradaDividendo;
        System.out.println("Indica un Divisor: ");
        int divisor = escaner.nextInt();
        for (int i = 0; ; i++) {
            if (dividendo > divisor) {
                dividendo -= divisor;
            } else {
                System.out.println(entradaDividendo + "/" + divisor + " =>\tEntero: " + i + "\tResto: " + dividendo);
                break;
            }
        }
    }

    private static void ejercicio25() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 25
                Indicar cuántas veces se ejecuta la 'acción' y con qué valor termina la variable 'contador'
                \033[36m
                Algoritmo:
                   int contador = 0;
                       while (contador < 10){
                           contador = contador + 1;
                           if(contador == 10){
                               accion;
                           }
                       }\u001b[0m
                Dado el código, el contador llegará a 10 y terminará ejecutando una vez la acción
                """);
        System.out.println(
        );
        int contador = 0;
        while (contador < 10) {
            contador = contador + 1;
            if (contador == 10) {
                System.out.println("ACCIÓN\nContador = " + contador);
            }
        }
    }

    private static void ejercicio26(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 26
                Leer los números enteros por teclados hasta que se introduzca el 0. Indicar cuántos son negativos
                """);
        int contadorNegativos = 0;
        System.out.println("Ingrese un número entero:");
        int numero26 = escaner.nextInt();
        while (numero26 != 0) {
            if (numero26 < 0) {
                System.out.println("Números negativos: " + ++contadorNegativos);
            } else {
                System.out.println("Número Positivo");
            }
            numero26 = escaner.nextInt();
        }
        System.out.println("Se han ingresado " + contadorNegativos + " números negativos.");
    }

    private static void ejercicio27(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 27
                La exponencial de un número real 'a' se puede aproximar con el sumatorio de una serie:
                    a)  Aproxiamar el resultado para n = 20
                    b)  Aproximar el resultado hasta que algún término de la serie sea menor que 0,001
                
                Ingresa el valor de 'a':
                """);
        int a = escaner.nextInt();
        int n27 = 1;
        long factorial27 = 1;
        double serieExponente = 0;
        while (n27 <= 20) {
            factorial27 *= n27;
            serieExponente += Math.pow(a, n27) / factorial27;
            n27++;
            //System.out.println(factorial27+" "+serieExponente+" "+n27);
        }
        serieExponente++; //Por el primer término n=0 [a^0/0! = 1/1 = 1]
        System.out.println("a)\tSe analiza e^" + a + " = " + serieExponente + " en 20 pasos.");

        n27 = 1;
        factorial27 = 1;
        serieExponente = 0;
        while (Math.pow(a, n27) / factorial27 >= 0.001) {
            factorial27 *= n27;

            serieExponente += Math.pow(a, n27) / factorial27;
            n27++;
            //System.out.println(factorial27 + " " + serieExponente + " " + n27);
        }
        serieExponente++; //Por el primer término n=0 [a^0/0! = 1/1 = 1]
        System.out.println("b)\tSe aproxima e^" + a + " = " + serieExponente + " en " + n27 + " pasos.");
    }

    private static void ejercicio28(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 28
                Escribir un algoritmo que calcule la raíz cuadrada de un número 'a' mediante la sucesión dada:
                
                Ingresa el valor de 'a':""");
        int a28 = escaner.nextInt();
        int contadorSucesion = 1;
        double sucesionRaiz = a28;
        System.out.println("X0 = " + a28);
        while (Math.abs(Math.pow(sucesionRaiz, 2) - a28) > 0.00000000000001) {
            sucesionRaiz = (sucesionRaiz + (a28 / sucesionRaiz)) / 2;
            System.out.println("X" + contadorSucesion + " = " + sucesionRaiz);
            contadorSucesion++;
        }
        System.out.println("\nResultado de la sucesión = " + sucesionRaiz + " en " + contadorSucesion + " pasos");
    }

    private static void ejercicio29(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 29
                Determina si un número entero es primo o no, indicando el número de divisiones que se realizaron para ello:
                
                Ingresa el valor del número a comprobar:""");
        int numeroPrimo = escaner.nextInt();
        int contadorPrimo = 2;
        while (numeroPrimo >= contadorPrimo && numeroPrimo % contadorPrimo != 0) {
            contadorPrimo++;
        }
        if (numeroPrimo != contadorPrimo) {
            System.out.println("El número " + numeroPrimo + " no es primo. Comprobado en " + contadorPrimo + " pasos.");
        } else {
            System.out.println("El número " + numeroPrimo + " es primo. Comprobado en " + contadorPrimo + " pasos.");
        }
    }

    private static void ejercicio30(Scanner escaner) {
        String reset = "\u001b[0m";         //Reset
        String terminado = "\033[32m";      //Green
        System.out.println("""
                    Boletin 2 3 | Ejercicio 30
                Comprueba el código del ejercicio 29 optimizando el cálculo del número primo dividiéndolo hasta la raíz cuadrada del número 'n' en vez de hasta el número 'n-1':
                
                Ingresa el número para comprobar si es primo:""");
        int numeroPrimo30 = escaner.nextInt();
        int contadorPrimo1 = 2;
        while (numeroPrimo30 >= contadorPrimo1 && numeroPrimo30 % contadorPrimo1 != 0) {
            contadorPrimo1++;
        }
        if (numeroPrimo30 != contadorPrimo1) {
            System.out.println("El número " + numeroPrimo30 + " no es primo. Comprobado con el primer método en " + contadorPrimo1 + " pasos.");
        } else {
            System.out.println("El número " + numeroPrimo30 + " es primo. Comprobado con el segundo método en " + contadorPrimo1 + " pasos.");
        }
        contadorPrimo1 = 2;
        while (Math.sqrt(numeroPrimo30) >= contadorPrimo1 && numeroPrimo30 % contadorPrimo1 != 0) {
            contadorPrimo1++;
        }
        if (Math.sqrt(numeroPrimo30) < contadorPrimo1) {
            System.out.println(terminado + "El número " + numeroPrimo30 + " es primo. Comprobado en " + contadorPrimo1 + " pasos." + reset);
        } else {
            System.out.println("El número " + numeroPrimo30 + " no es primo. Comprobado en " + contadorPrimo1 + " pasos.");
        }
    }

    private static void ejercicio31(Scanner escaner) {
        String terminado = "\033[32m";      //Green
        String reset = "\u001b[0m";         //Reset
        System.out.println("""
                    Boletin 2 3 | Ejercicio 31
                Mediante la mejora del código del ejercicio 30, calcular los números primos dentro del rango entre dos números ingresados por el usuario:
                
                Ingresa el número del principio del rango:""");
        int numeroPrimoInicio = escaner.nextInt();
        System.out.println("Ingresa el número del final del rango: ");
        int numeroPrimoFinal = escaner.nextInt();

        int contadorPrimo31 = 2;
        int numeroPrimo31 = numeroPrimoInicio;
        while (numeroPrimo31 <= numeroPrimoFinal) {
            while (Math.sqrt(numeroPrimo31) >= contadorPrimo31 && numeroPrimo31 % contadorPrimo31 != 0) {
                contadorPrimo31++;
            }
            if (Math.sqrt(numeroPrimo31) < contadorPrimo31) {
                System.out.println(terminado + "El número " + numeroPrimo31 + " es primo. Comprobado en " + contadorPrimo31 + " pasos." + reset);
            } else {
                System.out.println("El número " + numeroPrimo31 + " no es primo. Comprobado en " + contadorPrimo31 + " pasos.");
            }
            contadorPrimo31 = 2;
            numeroPrimo31++;
        }
    }

    private static void ejercicio32() {
        String terminado = "\033[32m";      //Green
        System.out.println("""
                    Boletin 2 3 | Ejercicio 32
                Muestra los 50 primeros números primos""");

        int divisorPrimo32 = 2;
        int contador32 = 0;
        int numeroPrimo32 = 1;
        while (contador32 <= 50) {
            while (Math.sqrt(numeroPrimo32) >= divisorPrimo32 && numeroPrimo32 % divisorPrimo32 != 0) {
                divisorPrimo32++;
            }
            if (Math.sqrt(numeroPrimo32) < divisorPrimo32) {
                System.out.println(terminado + "El número " + numeroPrimo32 + " es primo.");
                contador32++;
            }
            divisorPrimo32 = 2;
            numeroPrimo32++;
        }
    }

    private static void ejercicio33(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 33
                Calcular la cantidad mínima de billetes y monedas necesarias para cubrir el valor dado por el usuario:
                
                Ingresa el valor a comprobar:""");
        int entradaDinero = escaner.nextInt();
        int dinero = entradaDinero;
        int billete200 = 0;
        int billete100 = 0;
        int billete20 = 0;
        int billete10 = 0;
        int billete5 = 0;
        int moneda2 = 0;
        int moneda1 = 0;
        while (dinero != 0) {
            if (dinero >= 200) {
                billete200 = dinero / 200;
                dinero = dinero % 200;
            } else if (dinero >= 100) {
                dinero -= 100;
                billete100++;
            } else if (dinero >= 20) {
                dinero -= 20;
                billete20++;
            } else if (dinero >= 10) {
                dinero -= 10;
                billete10++;
            } else if (dinero >= 5) {
                dinero -= 5;
                billete5++;
            } else if (dinero >= 2) {
                dinero -= 2;
                moneda2++;
            } else {
                dinero -= 1;
                moneda1++;
            }
        }

        System.out.printf(entradaDinero + " serían: \n" +
                billete200 + " Billetes de 200€, \n" +
                billete100 + " Billetes de 100€, \n" +
                billete20 + " Billetes de 20€, " +
                billete10 + " Billetes de 10€, " +
                billete5 + " Billetes de 5€, " +
                moneda2 + " Monedas de 2€, " +
                moneda1 + " Monedas de 1€");
    }

    private static void ejercicio34(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 34
                Ingresa un número decimal y conviértelo a número binario:
                
                Ingresa un número decimal""");

        int contadorBinario = 0;
        int entradaDecimal = escaner.nextInt();
        int numeroDecimal = entradaDecimal;
        String binario = "";
        while (contadorBinario < 32) {
            if (numeroDecimal >= 0) {
                binario += "0";
            } else {
                binario += "1";
            }
            numeroDecimal = numeroDecimal << 1;
            contadorBinario++;
        }
        System.out.println(entradaDecimal + " = " + binario);
    }

    private static void ejercicio35() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 35
                Escribe un algoritmo que calcule a qué temperatura se consigue explotar una lata de refresco cerrada.
                """);
        double temperatura = 18;
        double presion;
        System.out.println("Temperatura Ambiente = " + temperatura + " ºC\n");
        do {
            presion = 0.00105 * Math.pow(temperatura++, 2) + 0.0042 * temperatura + 1.352;
            System.out.println("Temperatura = " + temperatura + " ºC\tPresión = " + presion + " atmósferas");
        } while (presion <= 3.2);
        System.out.println("Temperatura = " + temperatura + " ºC\tPresión = " + presion + " atmósferas | Ha explotado");
    }

    private static void ejercicio36(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 36
                Solicitar al usuario que teclee un número dentro de un bucle. Mostrar la suma de los números positivos introducidos y finalizar el bucle cuando teclee el 0.
                
                Ingresa un número entero:""");
        int numeroEntrada36 = escaner.nextInt();
        int sumaPositivos = 0;
        do {
            if (numeroEntrada36 > 0) {
                sumaPositivos += numeroEntrada36;
                System.out.println("Suma enteros positivos ingresados = " + sumaPositivos);
            } else {
                System.out.println("Suma enteros positivos ingresados = " + sumaPositivos);
            }
            System.out.println("\nIngresa otro número: ");
            numeroEntrada36 = escaner.nextInt();
        } while (numeroEntrada36 != 0);
        System.out.println("Fin de Sumas\nSuma enteros positivos ingresados = " + sumaPositivos);
    }

    private static void ejercicio37() {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 37
                ¿Para qué tres números enteros consecutivos su producto es 800 veces su suma?
                """);
        int numeroConsecutivo = 0;
        do {
            numeroConsecutivo++;
        } while (numeroConsecutivo * (numeroConsecutivo + 1) * (numeroConsecutivo + 2) != 800 * (numeroConsecutivo + (numeroConsecutivo + 1) + (numeroConsecutivo + 2)));
        System.out.println("Los números consecutivos que cumplen este caso son: \n - " + numeroConsecutivo + "\n - " + (numeroConsecutivo + 1) + "\n - " + (numeroConsecutivo + 2));
    }

    private static void ejercicio38() {

        System.out.println("""
                    Boletin 2 3 | Ejercicio 38
                Encuentra el primer valor 'n' para el que la siguiente suma excede a 10000:
                1+2+3+4+...+n.
                """);
        int sumaSerie38 = 0;
        int numero38 = 0;
        do {
            numero38++;
            sumaSerie38 += numero38;
            //System.out.println(numero38+": "+sumaSerie38);
        } while (sumaSerie38 < 10000);
        System.out.println("El número que excede la serie es: " + numero38);
    }

    private static void ejercicio39() {

        System.out.println("""
                    Boletin 2 3 | Ejercicio 39
                Encuentra los 3 primeros números perfectos""");
        int contadorPerfecto = 0;
        int numeroPerfecto = 6;
        int divisorPerfecto = 1;
        int sumatorioPerfecto = 0;
        do {
            if (divisorPerfecto >= numeroPerfecto) {
                if (sumatorioPerfecto == numeroPerfecto) {
                    System.out.println("El número " + numeroPerfecto++ + " es perfecto");
                    sumatorioPerfecto = 0;
                    divisorPerfecto = 1;
                    contadorPerfecto++;
                } else {
                    //System.out.println("El número " +numeroPerfecto++ +" no es perfecto");
                    numeroPerfecto++;
                    sumatorioPerfecto = 0;
                    divisorPerfecto = 1;
                }
            } else if (numeroPerfecto % divisorPerfecto == 0) {
                sumatorioPerfecto += divisorPerfecto++;
            } else {
                divisorPerfecto++;
            }

        } while (contadorPerfecto < 3);
    }

    private static void ejercicio40(Scanner escaner) {

        System.out.println("""
                    Boletin 2 3 | Ejercicio 40
                Aceptar números hasta que se introduzca uno que no esté en secuencia ascendente.
                Ingresa un número entero:""");
        int numeroActual = escaner.nextInt();
        int numeroPrevio = numeroActual - 1;

        while (numeroActual > numeroPrevio) {
            System.out.println("Ingresa otro:");
            numeroPrevio = numeroActual;
            numeroActual = escaner.nextInt();
        }
        System.out.println("Suficientes...");

    }

    private static void ejercicio41(Scanner escaner) {

        System.out.println("""
                    Boletin 2 3 | Ejercicio 41
                Igual que el anterior, pero especifica antes si es ascendente o descendente:
                
                ¿Es ascendente o descendente? ( a / d )""");
        String entrada = escaner.next();
        switch (entrada) {
            case "a":
                System.out.println("Ingresa un número:");
                int numeroActualAscendente = escaner.nextInt();
                int numeroPrevioAscendente = numeroActualAscendente - 1;

                while (numeroActualAscendente > numeroPrevioAscendente) {
                    System.out.println("Ingresa otro:");
                    numeroPrevioAscendente = numeroActualAscendente;
                    numeroActualAscendente = escaner.nextInt();
                }
                System.out.println("Suficientes...");
                break;
            case "d":
                System.out.println("Ingresa un número:");
                int numeroActualDescendente = escaner.nextInt();
                int numeroPrevioDescendente = numeroActualDescendente + 1;
                while (numeroActualDescendente < numeroPrevioDescendente) {
                    System.out.println("Ingresa otro:");
                    numeroPrevioDescendente = numeroActualDescendente;
                    numeroActualDescendente = escaner.nextInt();
                }
                System.out.println("Suficientes...");
                break;
            default:
                System.out.println("Repite por favor");
                break;
        }
    }

    private static void ejercicio42(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 42
                Ingresa una serie de números hasta cerrar con 0, entonces calcular la media de las distancias entre ellos:
                """);
        int numero1 = 0;
        int contador42 = 0;
        double distancia;
        double sumatorioDistancia = 0;
        double mediaDistancia;

        int numero2 = escaner.nextInt();
        while (numero2 != 0) {

            distancia = (double) Math.max(numero1, numero2) - Math.min(numero1, numero2);
            sumatorioDistancia += distancia;
            contador42++;
            numero1 = numero2;
            numero2 = escaner.nextInt();
        }
        if (contador42 == 0){
            System.out.println(contador42);
        } else {
            mediaDistancia = sumatorioDistancia / contador42;
            System.out.println("La media de las distancias es " + mediaDistancia);
        }
    }

    private static void ejercicio43(Scanner escaner) {
        System.out.println("""
                    Boletin 2 3 | Ejercicio 43
                Ejecuta el Algoritmo de Euclides para hayar el máximo común divisor de dos números dados:
                """);

        int a43 = escaner.nextInt();
        int b43 = escaner.nextInt();
        int q43;
        int r43 = 1;
        int numerador = Math.max(a43, b43);
        int denominador = Math.min(a43, b43);
        while (r43 != 0) {
            q43 = numerador / denominador;
            r43 = numerador % denominador;
            numerador = denominador;
            denominador = r43;
        }
        System.out.println("El máximo común divisor de " + a43 + " y " + b43 + " es " + numerador);
    }

    private static void ejercicio44() {
        System.out.println("\n\tBoletin 2 3 | Ejercicio 44\n");
        System.out.println("SIN EMPEZAR");
    }

    private static void ejercicio45() {
        System.out.println("\n\tBoletin 2 3 | Ejercicio 45\n");
        System.out.println("SIN EMPEZAR");
    }

    private static void ejercicio46() {
        System.out.println("\n\tBoletin 2 3 | Ejercicio 46\n");
        System.out.println("SIN EMPEZAR");
    }

    private static void ejercicio47() {
        System.out.println("\n\tBoletin 2 3 | Ejercicio 47\n");
        System.out.println("SIN EMPEZAR");
    }

    private static void ejercicio48() {
        System.out.println("\n\tBoletin 2 3 | Ejercicio 48\n");
        System.out.println("SIN EMPEZAR");

    }
}